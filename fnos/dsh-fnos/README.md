...
## 📐 版本命名规则（vX.Y.Z，与 Windows 版一致）

> 飞牛应用包版本号与 Windows 版遵循同一规则：

| 段位 | 含义 | 说明 |
|------|------|------|
| **X** | 大版本（重大更新/架构级迭代）| **0 为开发版本**；X 加一 = 重大更新 |
| **Y** | 功能集合版本 | 当前大版本下的新增功能集合 |
| **Z** | 修复迭代版本 | Y 功能版本下的修复/迭代版本（**从 1 开始，无 0**）|

**特殊规则：**
1. **Z = 4 → 紧急更新**：重大漏洞/安全问题跳过常规迭代直接发布修复版。
   - 例：`0.2.2` 发现大漏洞 → 发布 `0.2.4` 修复；`0.11.18` 出问题 → `0.11.24`
2. **Z 末尾为 0 → 当前 Y 版本的最终版本**：发布后不再有后续。
   - 例：`0.2.10`、`0.8.20` 为最终版本（有了 `0.2.10` 就不会有 `0.2.11`）
3. **Z 无 0**：`0.8.0` ❌ → `0.8.1` ✅；`1.0.0` ❌ → `1.0.1` ✅（Z 末尾 0 仅用于最终版本标记）

**示例：** `dsh-0.2.4.fpk` = 0.2 功能线下的紧急修复版（Z=4）；`dsh-0.2.10.fpk` = 0.2 功能线最终版。
