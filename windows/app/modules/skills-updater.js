 ...
 *  - 数据下载：拉服务器 GET {apiUrl}/data（返回完整技能数组）→ 存本地缓存
 ...
 *  - readShellConfig  读取 config.json（取 skillsUpdate.apiUrl）
 */
...
function createSkillsUpdater(deps) {
  ...
  /** 从 config.json 读技能库接口地址（单源）；无配置返回 null */
  function apiBase() {
    try {
      const cfg = readShellConfig();
      const u = cfg.skillsUpdate && cfg.skillsUpdate.apiUrl;
  ...
  /** 加载本地缓存（userData/skills-market-cache.json） */
  function loadCache() {
    try {
      ...
      if (fs.existsSync(file)) {
        ...
        // 校验 data 必须为技能数组，否则视为无缓存（需重新下载）
        if (parsed && parsed.version && Array.isArray(parsed.data)) {
          cached = parsed;
          appendLog('info', `技能库缓存已加载：v${parsed.version}（${parsed.data.length} 个技能）`);
          return cached;
        }
      }
    } catch (err) {
      appendLog('warn', `加载技能库缓存失败：${err.message}`);
    }
    return null;
  }

  /** 保存缓存到 userData/skills-market-cache.json（原子写入） */
  function saveCache(version, data, updated) {
    try {
      ...
      appendLog('info', `技能库缓存已保存：v${version}（${data.length} 个技能）`);
    } catch (err) {
      appendLog('error', `保存技能库缓存失败：${err.message}`);
    }
  }

  /** 获取技能库市场数据（仅缓存；无缓存 → 标记需要下载） */
  function getData() {
  ...
  async function queryInfo() {
    ...
    try {
      ...
      appendLog('warn', `技能库版本检测失败：${err.message}`);
    }
    return info;
  }
  ...
  async function downloadData() {
    ...
    try {
      ...
      appendLog('error', `下载技能库失败：${err.message}`);
      return { ok: false, reason: 'fetch-failed', version: null };
    }
  }

  /** 检查技能库更新：有新版或尚无缓存 → 下载数据落缓存；无更新 → 保持缓存。 */
  ...
  async function checkUpdatesOnStart() {
    ...
    try {
      appendLog('info', '开始检查技能库更新...');
      const r = await forceUpdate();
      if (!r.ok) {
        appendLog('warn', `技能库检查失败：${r.reason}`);
      } else if (!r.updated) {
        appendLog('info', `技能库已是最新版本：v${r.info.latest}`);
      }
    } catch (err) {
      appendLog('error', `检查技能库更新失败：${err.message}`);
    }
  }
