...
const out = {
  ...
  // v1.1.1 二轮（用户反馈 + Issue#1 + 26 方案七/八）：
  // 插件市场深色模式
  ...
  // v1.1.1 三轮（用户反馈）：确认弹窗免责声明红色加粗 + 去掉手机App类比 + 刷新按钮
  ...
  // 帮助文档：应用内窗口 + 本地优先 + 后台静默远程同步（v1.1.1 三轮，用户反馈）
  ...
  // 首次安装体验（用户反馈）：启动页 30~40 分钟提示 + 等待计时
  ...
  // v1.1.2（用户反馈）：公告条点击跳转公告窗口（不再直达二维码）
  ...
  // v1.1.2（用户反馈）：启动界面（loading）显示「打开帮助文档」按钮
  ...
  // v1.1.2（用户反馈）：问题1 —— setWindowOpenHandler 不放行本地回环，
  ...
  // v1.1.3（用户指令）：1.1.1 未发布、1.1.2 删除，更新内容全部合并到 1.1.3
