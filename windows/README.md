...
- **普通用户（安装包）**：无需安装任何东西——壳自带 Node 24（随包分发），
  双击安装即用，不受系统环境影响
- **开发者（源码运行）**：需要 Node.js **≥ 20.17**（内置 npm 12 依赖；
  低于此版本会报 `crypto.randomUUID is not a function`）
  - 推荐直接下载安装包使用（体验一致，省去环境配置）
  - 若确实要跑源码：`npm start` 前请先确认 `node -v` ≥ v20.17
  - v1.1.1 起自动兜底：检测到系统 Node 过旧时自动回落 Electron 内置 Node（不再报错）
